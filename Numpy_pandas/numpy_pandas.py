...
def f1(X1,coef1,X2,coef2,seed1,seed2,seed3,shape1,shape2):
	...
	B = np.random.rand(0,100)
	ans = W1 * (X1 ** coef1) + W2 * (X2 ** coef2) + B
	return ans
...
def fill_with_group_average(df, group, column):
    ...
    df.groupby(group)[column].transform(lambda x:x.fillna(np.mean(x)))
    print(df)
    return df
...
def get_rows_greater_than_avg(df, column):
    ...
    df=None
    df = pd.read_csv('example.csv')
    df[column] = df[df.columns[df.column > mean(df.column)]]
    return df
