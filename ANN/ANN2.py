...
class Tensor:
    ...
    __add__: Invoked when L operand of + is Tensor
    ...
    Tensor-Tensor
    __matmul__: Invoked when L operand of @ is Tensor
    ...
    def grad_add(self, gradients=None):
        ...
        if gradients is None:
                l= np.ones_like(self.arr)
                r= np.ones_like(self.arr)
                return (l, r)
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'matmul':
            ar = self.grad_matmul(gradients)

        if self.history[0] == 'add':
            ar = self.grad_add(gradients)
    ...
    def grad_matmul(self, gradients=None):
        ...
        """
        # TODO
        G = np.ones_like(self.arr)
        if gradients is None:
            L = np.matmul(G, self.history[2].arr.transpose())
            R = np.matmul(self.history[1].arr.transpose(), G)
            return (L, R)

        L = np.matmul(gradients, self.history[2].arr.transpose())
        R = np.matmul(self.history[1].arr.transpose(), gradients)
        return (L, R)
